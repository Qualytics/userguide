...
A field that remains fully operational (profiled, scanned, and quality-checked) but whose actual values are hidden across the platform by default. Users with Editor permission can reveal masked values, and every access is recorded in the masking audit log. Learn more in [Field Masking](fields/field-status/concepts/field-masking.md).
