...
| **4** | **Table** | Filters anomalies based on the table where they occurred, useful for investigating issues in a specific dataset. |
| **5** | **Field** | Filters anomalies based on the column in the table where the issue was found, so you can see which field caused the problem. |
| **6** | **Check** | Filters anomalies based on the check that generated them, allowing you to trace back to the check configuration. |
| **7** | **Tags** | Filters anomalies by tags that have been applied to checks or rules, allowing for easier grouping or categorization. |
