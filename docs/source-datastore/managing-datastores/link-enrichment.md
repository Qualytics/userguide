...
!!! note "Permission Required"
    You need the **Member** user role and **Editor** team permission on the datastore to link an enrichment datastore. See the [Permissions](../enrichment-datastore/permissions.md){:target="_blank"} page for details.

!!! info "Link During Datastore Creation"
    You can also link an enrichment datastore during the datastore creation wizard. See the [Link on Datastore Creation](../enrichment-datastore/link-during-creation.md){:target="_blank"} documentation.

## Enrichment Settings

The **Link Enrichment Datastore** modal contains the following fields that you will configure during the linking process:

![modal-fields-reference](../../assets/source-datastores/enrichment-datastore/managing/link-enrichment/modal-fields-reference.png)

| REF. | FIELD | DEFAULT | RANGE | DESCRIPTION |
|:---:|:---|:---:|:---:|:---|
| 1 | Prefix | Auto-generated | Max 60 chars | A prefix added to all enrichment table names to distinguish them from source tables. Auto-generated from the datastore name, normalized to lowercase with underscores (e.g., `_healthcare_analytics`). Each source datastore linked to the same enrichment datastore **must have a unique prefix** to avoid table name conflicts. |
...
!!! warning "Side Effects"
    Once linked, all future Scan operations will write results to the enrichment datastore. For details on how enrichment works, storage impact, and remediation strategies, see the [Enrichment Introduction](../enrichment-datastore/introduction.md){:target="_blank"} page.

![step-5-modal-configured](../../assets/source-datastores/enrichment-datastore/managing/link-enrichment/step-5-modal-configured.png)

**Step 6**: A success message will confirm that the datastore has been updated successfully.

![step-6-success](../../assets/source-datastores/enrichment-datastore/managing/link-enrichment/step-6-success.png)
...
!!! info "Unlink Enrichment Datastore"
    To remove the enrichment link, see the [Unlink Enrichment Datastore](unlink-enrichment.md){:target="_blank"} documentation.
