...
### Who can mask, exclude, or restore fields?

All field status operations require two layers of authorization:

1. **User Role**: Must be at least a **Member** (organization-level)
2. **Team Permission**: Must have **Editor** permission on the field's associated datastore (resource-level)

### Who can view masked field values?

Users with **Editor** team permission or above can request to reveal masked values. Every reveal action is recorded in the masking audit log.

### Who can access the masking audit log?

Only users with the **Admin** user role can access the masking audit log.

### Does the permission requirement differ by operation?

No. All mutating field status operations — mask, unmask, exclude, restore, delete, merge, and bulk operations — require the same permission: **Member** role + **Editor** team permission on the datastore.
