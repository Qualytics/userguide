...
- **Quality Checks**: Associated checks are set to Discarded; [Expected Schema](/data-quality-checks/expected-schema-check.md) checks are updated to remove the excluded field from their definition
...
!!! warning
    This action is reversible through the [restore operation](restore-a-field.md){:target="_blank"}, but previously archived quality checks will **not** be automatically restored.
